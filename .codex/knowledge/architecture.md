...
- 已注册设备按共享配置周期发送 MANSCDP \`MESSAGE Keepalive\`，并将发送成功、连续失败次数和在线状态投影到运行时快照；共享 UDP 接收循环统一识别平台 SIP 方法、CmdType、Call-ID、Expires 和通道归属，平台请求回送关联头部的 \`200 OK\`，对 Catalog、DeviceInfo、DeviceStatus、DeviceControl、RecordInfo 生成基础 Query/Response。SUBSCRIBE 建立、刷新、取消和过期由内存订阅状态机维护，并在建立有效订阅后发送基础 \`NOTIFY\`。Alarm 与 Mobile Position 通过设备管理页右侧通道卡片单通道手动触发，设备控制和 PTZ 命令均通过统一业务队列发送并更新运行时状态，报文和响应进入交互日志。
