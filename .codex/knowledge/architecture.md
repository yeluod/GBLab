...
- 已注册设备按共享配置周期发送 MANSCDP \`MESSAGE Keepalive\`；共享 UDP 接收循环识别平台 SIP 请求并回送 \`200 OK\`，对 Catalog 查询自动生成设备及动态通道目录响应。Alarm 与 Mobile Position 通过设备管理页右侧通道卡片单通道手动触发，触发按钮仅在设备已注册时可用，报文和响应进入交互日志。
